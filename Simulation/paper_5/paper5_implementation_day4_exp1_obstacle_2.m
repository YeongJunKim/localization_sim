...
for ct = 2:app.iteration
        ...
        if(app.digraph.Nodes.Type{ag} == "known")
            ...
            if ct > 120 && ct < 200
               measurement(end,1) = measurement(end,1) + normrnd(0, 0.01); 
                ...
                measurement(end,1) = measurement(end,1) + normrnd(0, 0.005);
            end
            end
            estimator{app.index_RDFIR, ag}.estimate2(ag,u,measurement,app.adj_full, pj_DRFIR);
            if ag == 3
            if ct > 120 && ct < 200
               measurement(end,1) = measurement(end,1) + normrnd(0, 1); 
            else
    ...
    for ag = 1:app.agent_num
        if(app.digraph.Nodes.Type{ag} == "known")
            ...
            result.agent(ag).trajectory_real(3,ct) = x1(3).*0.2 + x2(3)*0.1 + x3(3) * 0.7 + normrnd(0, 0.15);
%             pj_DRFIR(:,ag) = pj_DRFIR(:,ag) * 0.7 + result.agent(ag).trajectory_real(1:2,ct) * 0.3;
        end
    end
end

%% figure estimated trajectory
fig_user_input_trajectory = figure(101);
...
%%
fig_user_input_trajectory = figure(98);
fig_user_input_trajectory.Name = 'Trajectory_DFMERM only';
clf;
fig_input_selection_ax = axes;
rectangle_points = zeros(2,5);
rectangle_points(:,1) = [1.52 2.8];
...
fig_user_input_trajectory = figure(103);
fig_user_input_trajectory.Name = 'Estimation Error DREKF';
clf;
for i = 1:app.agent_num
%     fprintf("[%d] index \r\n", i);
    if(app.digraph.Nodes.Type{i} == "known")
...
fig_user_input_trajectory = figure(104);
fig_user_input_trajectory.Name = 'Sume of Estimation Error';
clf;
...
plot(x,y1(2,:),'-d','MarkerSize',10, 'DisplayName', "DFMERM(p^x, p^y)"); hold on;
plot(x,y2(1,:),'-+','MarkerSize',10, 'DisplayName', "KF-based p^\theta"); hold on;
plot(x,y2(2,:),'-x','MarkerSize',10, 'DisplayName', "DFMERM p^\theta"); hold on;
xlim([0 6]);
ylim([0 0.3]);
...
fig_RMSE = figure(106);
fig_RMSE.Name = 'Trajectory Summary';
clf;
markersize = 7;
boxlinewidth = 2;
linewidth = 1;
